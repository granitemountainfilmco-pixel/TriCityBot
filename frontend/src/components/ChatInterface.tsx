  ...
  useEffect(() => {
    ...
    recognitionRef.current.onresult = (event: any) => {
      ...
      for (let i = event.resultIndex; i < event.results.length; ++i) {
        // SPACE INJECTOR: Prevents 'rtx' '5090' '500' from becoming 'rtx5090500'
        transcript += event.results[i][0].transcript + ' ';
      }

      if (event.results[event.results.length - 1].isFinal) {
        const cleanText = transcript.trim().toLowerCase();
        const triggers = ['add', 'check', 'research', 'remove', 'delete', 'stock', 'have'];
    ...
    recognitionRef.current.onend = () => { if (!isProcessing) recognitionRef.current.start(); };
  ...
  return (
    <div className="bg-slate-900 border-2 border-blue-500/20 rounded-3xl p-8 w-full max-w-2xl shadow-2xl">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-xl font-bold text-blue-500 tracking-tighter uppercase">Shop OS v3.5</h1>
      ...
      <p className="text-[10px] text-slate-600 font-bold uppercase text-center tracking-widest">
        {isProcessing ? "Processing..." : "Mic Active // Say Add, Remove, or Research"}
      </p>
    </div>
  );
}
